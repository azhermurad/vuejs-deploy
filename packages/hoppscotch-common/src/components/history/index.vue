<template>
  <div>
    ...
    <div class="flex flex-col">
      <component
        v-for="(filteredHistoryGroup, filteredHistoryGroupIndex) in requests"
        :is="HistoryRestCard"
        :id="filteredHistoryGroupIndex"
        :key="`entry-${filteredHistoryGroupIndex}`"
        :entry="filteredHistoryGroup"
        :show-more="showMore"
        @use-entry="useHistory(toRaw(filteredHistoryGroup))"
      />
      <!-- <details
...
import { computed, ref, Ref, toRaw, onMounted } from "vue"
...
onMounted(() => {
  console.log(filteredHistoryGroups.value)
})
...
const parseRequests = computed(() => {
  let req = JSON.stringify(requests)
...
const filteredHistoryGroups = computed(() =>
  groupBy(
    ...
    (input) => getAppropriateURL(input.entry)
  )
)
...
const useHistory = (entry: RESTHistoryEntry) => {
  ...
  console.log(entry, "entryyyy")
  setRestReq(entry as HoppRESTRequest)
  clickedHistory.value = entry
}
...
const saveCurrentRequest = (saveCtx: HoppRequestSaveContext | null) => {
  ...
  if (saveCtx.originLocation === "user-collection") {
    try {
      ...
      clickedHistory.value &&
        setRestReq(clickedHistory.value as HoppRESTRequest)
      setRESTSaveContext(null)
